...
public class CustomerDAOCollImpl implements CustomerDAO {
    ...
    public boolean addToOrderProcessingList(Customer... customer) {
        for (Customer c: customer) {
            ordersList.add(c);
            System.out.println("\n***" + c.getCustomerFirstName() + " " + c.getCustomerLastName() + " has been added to the list of orders***");
        }
        return true;
    }

    @Override
    public boolean removeFromOrderProcessingList(Customer... customer) {
        for (Customer c: customer) {
            ordersList.remove(c);
            System.out.println("\n***" + c.getCustomerFirstName() + " " + c.getCustomerLastName() + " has been removed from the list of orders***");
        }
        return true;
    }

    @Override
    public Customer getFromOrderProcessingListByName(String firstName, String lastName) {
        for(Customer c: ordersList) {
            if (c.getCustomerFirstName().equals(firstName) && c.getCustomerLastName().equals(lastName)) {
                System.out.println("\n***Getting " + c.getCustomerFirstName() + " " + c.getCustomerLastName() + " from the list of orders***");
                return c;
            }
        }
        return null;
    ...
    @Override
    public Customer getFromOrderProcessingListById(Long id) {
        for(Customer c: ordersList) {
            if (c.getId() == id) {
                System.out.println("\n***Getting customer with ID \"" + c.getId() + "\" from the list of orders***");
                return c;
            }
        }
        return null;
    ...
    public void showAllOrdersById() {
        System.out.println("\n***Displaying the list of orders sorted by ID***");
    ...
    public void showAllOrdersByLastName() {
        System.out.println("\n***Displaying the list of orders sorted by last name***");
    ...
    public void showAllOrdersByInvoice() {
        System.out.println("\n***Displaying the list of orders sorted by invoice***");
    ...
    @Override
    public void saveOrderProcessingList(String saveType){
        if (saveType.toLowerCase().startsWith("ser")) {
            ...
            System.out.println("\n***Marshalling list of orders to " + fileName + "***");
            try {
    ...
    @Override
    public List<Customer> loadOrderProcessingList(String fileType) {
        List<Customer> newOrdersList = Collections.EMPTY_LIST;
        if (fileType.toLowerCase().startsWith("ser")) {
            final String fileName = "ordersList.ser";
            System.out.println("\n***Deserializing list of orders from " + fileName + "***");
            try {
            ...
            final String fileName = "ordersList.xml";
            System.out.println("\n***Unmarshalling list of orders from " + fileName + "***");
            try {
                XStream xStream = new XStream();
                xStream.alias("Customer", Customer.class);
                xStream.alias("Product", Product.class);
                newOrdersList = (List<Customer>) xStream.fromXML(new FileInputStream(fileName));
            } catch (FileNotFoundException e) {
                e.printStackTrace();
            }
    ...
    class idSorterComparator implements Comparator<Customer> {
        @Override
    ...
    class lastNameSorterComparator implements Comparator<Customer> {
        @Override
        public int compare(Customer c1, Customer c2) {
    ...
    class invoiceSorterComparator implements Comparator<Customer> {
        @Override
        public int compare(Customer c1, Customer c2) {
